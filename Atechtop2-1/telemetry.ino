...
//Oximeter and Temperature 
    ...
    if(millis() > nextOxygenReading){
      nextOxygenReading = millis() + 60000;   // current time + 1 minute, data type 32-bit unsigned
     ...
     // logarithmic calculation to oxigen saturation 
      ...
      sendWordPacket(OXIMETER_SENSOR_ID, oxg); // distance sensor data package (need to commant it out when not connecting with arx frame work)
      sendWordPacket(TEMP_SENSOR_ID, tempe); // distance sensor data package; need to commond it out when outside of arx file 
      #if debug 
